...
class screenGraphics(object):
    ...
    def welcomeScreen(self):
        ...
        welcomeTurtle.write("Welcome to my", move=False, align="center",
                            font=("Arial", int(BOARD_DIMENSION // 10), "normal"))
        welcomeTurtle.setpos(0, BOARD_DIMENSION / 3 - (BOARD_DIMENSION / 9))
        welcomeTurtle.write("TicTacToe Game", move=False, align="center",
                            font=("Arial", int(BOARD_DIMENSION // 10), "normal"))

        newGameButton = createButton(-BOARD_DIMENSION // 9.8, -BOARD_DIMENSION // 2, "New Game",
                                     "drawScreens.newGame()", 20)
    ...
    def boardScreen(self):
        ...
        for i in range(4):
            ...
            # Saves the coordinates of the main_turtle, so the main_turtle goes to the top left corner before drawing
            # the rest of ther board
            if i == 2:
                gotoX = boardTurtle.xcor()
                gotoY = boardTurtle.ycor()

        # Goes to the saved coordinates before starting to draw the little square, so it is easier to work with
        # game_board
        boardTurtle.up()
        ...
        for row in range(3):
            ...
            boardTurtle.up()
            boardTurtle.setpos((boardTurtle.xcor() - (BOARD_DIMENSION / 3) * 3),
                               (boardTurtle.ycor() - (BOARD_DIMENSION / 3)))
            boardTurtle.down()

        newGameButton = createButton(-BOARD_DIMENSION // 9.8 + (-BOARD_DIMENSION // 5), -BOARD_DIMENSION // 2,
                                     "New Game", "drawScreens.newGame()", 20)
        BUTTONS.append(newGameButton)

        toWelcomeScreenButton = createButton(-BOARD_DIMENSION // 9.8 - (-BOARD_DIMENSION // 5), -BOARD_DIMENSION // 2,
                                             "Main Menu", "drawScreens.welcomeScreen()", 20)
    ...
    def endScreen(self, text_to_write):
        ...
        if text_to_write == "It's a tie":
            endTurtle.write(text_to_write, move=False, align="center",
                            font=("Arial", int(BOARD_DIMENSION // 10), "normal"))
        else:
...
class createButton(object):
    ...
    def checkClicked(self, x, y):
        ...
        if (int(self.x_cor) < x < int(self.x_cor + self.width)) and (
                int(self.y_cor) > y > int(self.y_cor - self.height)):
            return True
        else:
            return False
...
def executeMove(row, col, selectedTurtle, text_to_write):
    """
    This function executes the move and gets called when ever the ai or the player makes a move and appends the piece
    character to the game_board at that location :param row: where to go :param col: where to go :param
    selectedTurtle: either aiTurtle or playerTurtle :param text_to_write:  either ai_text_to_write or
    player_text_to_write :return: None
    """
    ...
    selectedTurtle.up()
    selectedTurtle.goto((BOX_LOCATIONS[row][col][0]) + BOARD_DIMENSION / 6,
                        BOX_LOCATIONS[row][col][1] - BOARD_DIMENSION / 3)
    selectedTurtle.down()
...
def click(x, y):
    """
    This function checks if the user has clicked on a specific button by calling the checkClicked method for every
    button in the BUTTONS list :param x: x-coordinate :param y: y-coordinate :return: None
    """
    ...
    if ((BOX_LOCATIONS[0][0][0] < x < (BOX_LOCATIONS[2][2][0]) + BOARD_DIMENSION / 3)) and (
            (BOX_LOCATIONS[0][0][1] > y > (BOX_LOCATIONS[2][2][1]) - BOARD_DIMENSION / 3)):

        # Evaluates if the player has clicked inside one of the squares
        for row in range(3):
            for col in range(3):
                if (BOX_LOCATIONS[row][col][0] < x < BOX_LOCATIONS[row][col][0] + BOARD_DIMENSION / 3) and (
                        BOX_LOCATIONS[row][col][1] - BOARD_DIMENSION / 3 < y < BOX_LOCATIONS[row][col][1]):
